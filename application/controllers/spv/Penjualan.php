...
class Penjualan extends CI_Controller
{
  ...
  public function lap_artikel()
  {
    ...
    where ts.status ='1' AND tt.id_spv = '$id_spv' GROUP BY ts.id_produk")->result();
    $this->template->load('template/template', 'spv/penjualan/lap_artikel', $data);
  }
  ...
  public function lap_toko()
  {
    ...
    $id_spv = $this->session->userdata('id');
    $data['toko'] = $this->db->query("SELECT * from tb_toko where status ='1' AND id_spv = '$id_spv'")->result();
    $this->template->load('template/template', 'spv/penjualan/lap_toko', $data);
  }
