...
class Barang extends CI_Controller {
  ...
  public function index()
  {
    ...
    $data['avatar'] = $this->M_admin->get_data_gambar('tb_upload_gambar_user',$this->session->userdata('name'));
      $this->template->load('template/template', 'admin_mv/barang/index', $data);
  }
    ...
    public function proses_update()
  {
    ...
    if($this->form_validation->run() == TRUE)
    {
      ...
      redirect(base_url('adm_mv/barang'));
    }else{
    ...
    public function proses_tambah() {
      ...
      if($this->form_validation->run() == TRUE )
      {
          ...
          $cek = $this->db->query("SELECT * FROM tb_produk WHERE kode = '$kode' AND status = '1'")->num_rows();
          if ($cek>0) {
            tampil_alert('info','Gagal','Artikel sudah ada, Silahkan masukkan kode yang lain!');
            redirect(base_url('adm_mv/barang'));
          }else{
