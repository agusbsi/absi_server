              ...
              <tbody>
                ...
                ?>
                  <tr>
                    ...
                    <td>
                      <button type="button" class="btn btn-<?= ($t->status == 12 ? "success" : "info") ?> btn-sm" onclick="getdetail('<?php echo $t->id_retur; ?>','<?= $t->nama_toko ?>','<?= $t->created_at ?>')" data-toggle="modal" data-target=".bd-example-modal-lg"><i class="fa fa-eye"></i> <?= ($t->status == 12 ? "Proses" : "Detail") ?></button>
                    </td>
                  </tr>
                <?php endforeach ?>
              </tbody>
...
<div class="modal fade bd-example-modal-lg" tabindex="-1" role="dialog" aria-labelledby="myLargeModalLabel" aria-hidden="true">
  <div class="modal-dialog modal-xl">
    <form action="<?= base_url('adm/Toko/approveToko') ?>" method="POST">
...
<script>
  function getdetail(id, toko) {
    ...
    $.ajax({
      url: '<?= base_url('adm/Toko/getdataRetur') ?>', // Ganti dengan URL ke fungsi controller yang mengambil data artikel
      type: 'GET',
      data: {
        id_retur: id
      },
  ...
  $('#btn_tolak').click(function(e) {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        location.href = "<?= base_url('adm/Toko/tolakToko/') ?>" + no_retur; // Perbaikan URL
      }
    })
  });
</script>