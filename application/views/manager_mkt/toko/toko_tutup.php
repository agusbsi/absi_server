              ...
              <tbody>
                ...
                ?>
                  <tr>
                    ...
                    <td>
                      <button type="button" class="btn btn-<?= ($t->status == 11 ? "success" : "info") ?> btn-sm" onclick="getdetail('<?php echo $t->id_retur; ?>','<?= $t->nama_toko ?>','<?= $t->created_at ?>')" data-toggle="modal" data-target=".bd-example-modal-lg"><i class="fa fa-eye"></i> <?= ($t->status == 11 ? "Proses" : "Detail") ?></button>
                    </td>
                  </tr>
                <?php endforeach ?>
              </tbody>
...
<div class="modal fade bd-example-modal-lg" tabindex="-1" role="dialog" aria-labelledby="myLargeModalLabel" aria-hidden="true">
  <div class="modal-dialog modal-xl">
    <form action="<?= base_url('mng_mkt/Toko/approveToko') ?>" method="POST">
          ...
          <div class="row">
            ...
            <div class="col-md-4">
              <div class="form-group">
                ...
                <textarea name="catatan_mm" id="catatan_mm" class="form-control" rows="3" cols="100%" required></textarea>
              </div>
            </div>
          </div>
        </div>
...
<script>
  function getdetail(id, toko) {
    ...
    $.ajax({
      url: '<?= base_url('mng_mkt/Toko/getdataRetur') ?>', // Ganti dengan URL ke fungsi controller yang mengambil data artikel
      type: 'GET',
      data: {
        id_retur: id
      },
      success: function(data) {
        ...
        if (data.status != 11) {
          ...
          catatan_mm.setAttribute('readonly', 'readonly');
        } else {
          // Jika status adalah 10, hapus atribut readonly pada textarea
          catatan_mm.removeAttribute('readonly');
        }
    ...
    if (catatan == "") {
      ...
      Swal.fire({
        ...
        if (result.isConfirmed) {
          location.href = "<?= base_url('mng_mkt/Toko/tolakToko/') ?>" + no_retur + '/' + catatan; // Perbaikan URL
        }
      })
    }
  });
