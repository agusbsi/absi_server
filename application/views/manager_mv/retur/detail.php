          ...
          <div class="invoice p-3 mb-3">
                  ...
                  <tbody>
                    ...
                    ?>
                      <tr>
                        ...
                        <td><?= $d->qty ?></td>
                        <td>
                          <button type="button" class="btn btn-sm btn-outline-primary btn-foto" data-id_produk="<?= $d->kode_produk ?>" src="<?= base_url('assets/img/retur/' . $d->foto) ?>"><i class="fas fa-eye"></i> Lihat</button>
                        </td>
                        <td><?= $d->keterangan ?></td>
                        <td><?= $d->catatan ?></td>
                      </tr>
            ...
            <div class="row">
              <div class="col-md-6">
                ...
                <textarea rows="3" class="form-control form-control-sm"  readonly><?= $retur->catatan_leader ?></textarea>
              </div>
              <div class="col-md-6">
                <p class="lead">Catatan MV:</p>
                <?php if ($retur->status == 1) { ?>
                  <textarea name="catatan_mv" id="catatan-mv" rows="3" class="form-control form-control-sm"></textarea>
                  <input type="hidden" id="id_retur" value="<?= $retur->id ?>">
                <?php } else {
                  echo $retur->catatan_mv;
                } ?>
            ...
            <?php if ($retur->status == 1) { ?>
              <div class="row no-print">
                <div class="col-12">
                     <a href="<?= base_url('sup/Retur')?>"  class="btn btn-sm btn-danger float-right" style="margin-right: 2px;">
            ...
            <?php } else { ?>
              <div class="row no-print">
                <div class="col-12">
                  <a href="<?= base_url('sup/retur') ?>" class="btn btn-sm btn-danger float-right"> <i class="fas fa-arrow-left"></i>Kembali</a>
                  <a type="button" onclick="printDiv('printableArea')" target="_blank" class="btn btn-sm btn-default float-right" style="margin-right: 5px;">
                    <i class="fas fa-print"></i> Print </a>
                </div>
              </div>
...
<script>
  function validateCatatanAndAction(action) {
    var catatan = document.getElementById('catatan-mv').value.trim();

    if (catatan === '') {
      Swal.fire({
        title: 'Belum Lengkap',
        text: 'Catatan MV tidak boleh kosong.!',
        icon: 'warning'
      });
    } else {
  ...
  function saveCatatanAndPerformAction(catatan, action) {
    ...
    $.ajax({
      url: '<?= base_url('sup/Retur/tindakan') ?>',
      type: 'POST',
      data: {
        catatan: catatan,
        action: action,
      ...
      success: function(response) {
        if (response === 'setuju') {
          Swal.fire({
            title: 'Berhasil',
            text: 'Data retur berhasil di proses.',
            icon: 'success',
