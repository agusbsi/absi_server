          ...
          <div class="row no-print mb-5">
            <div class="col-12">
              <?php if ($mutasi->status == 0) { ?>
                <div class="col-12">
                  <a href="<?= base_url('sup/Mutasi') ?>" class="btn btn-secondary float-right btn-sm" style="margin-left: 5px;"><i class="fas fa-times-circle"></i> Close </a>
                  <button type="button" class="btn btn-danger float-right btn_tolak btn-sm"><i class="fas fa-times-circle"></i> Tolak Semua</button>
                  -
                  <button type="button" class="btn btn-success float-right btn_terima btn-sm" style="margin-right: 5px;"><i class="fas fa-check"></i> Approve </button>
                </div>
              <?php } else { ?>
                ...
                <a href="<?= base_url('sup/Mutasi') ?>" class="btn btn-danger float-right btn-sm" style="margin-right: 5px;"><i class="fas fa-times-circle"></i> Close </a>
              <?php } ?>
            </div>
          </div>
        </div>
  ...
  $(document).ready(function() {
      ...
      if (id == 0) {
        ...
        Swal.fire({
          ...
          if (result.isConfirmed) {
            $.ajax({
              url: "<?= base_url('sup/Mutasi/approve') ?>",
              dataType: 'text',
              data: {
                id: id,
                id_mutasi: id_mutasi
              ...
              success: function() {
                ...
                Swal.fire({
                  ...
                  if (result) {
                    ...
                    location.href = "<?php echo base_url('sup/Mutasi') ?>";
                  } else {
    ...
    $('.btn_tolak').click(function() {
      ...
      Swal.fire({
        ...
        if (result.isConfirmed) {
          $.ajax({
            url: "<?= base_url('sup/Mutasi/reject') ?>",
            dataType: 'text',
            data: {
              id_mutasi: id_mutasi
            },
